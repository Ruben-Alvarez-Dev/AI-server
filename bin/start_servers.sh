...
# Starts Memory Server + LLM Server
...
echo "================================="
echo -e "${NC}"
echo -e "${CYAN}Starting Memory Server + LLM Server${NC}"
...
MEMORY_DIR="${BASE_DIR}/apps/memory-server"
LLM_DIR="${BASE_DIR}/apps/llm-server"
LOGS_DIR="${BASE_DIR}/monitoring/logs"
...
if [ ! -d "$MEMORY_DIR" ]; then
    echo -e "${RED}❌ Memory Server directory not found: $MEMORY_DIR${NC}"
    exit 1
fi
...
if [ ! -d "venv" ]; then
    echo -e "${YELLOW}📦 Creating virtual environment for Memory Server...${NC}"
    python3 -m venv venv
fi

# Start Memory Server in background
echo -e "${CYAN}   Starting Memory Server...${NC}"
...
if ! wait_for_service "http://localhost:8001/health" "Memory Server"; then
    echo -e "${RED}❌ Failed to start Memory Server${NC}"
...
echo -e "   • Memory Server:     ${CYAN}http://localhost:8001${NC}"
...
trap 'echo -e "\n${YELLOW}🛑 Shutting down servers...${NC}"; kill $MEMORY_PID 2>/dev/null || true; kill $LLM_PID 2>/dev/null || true; pkill -f "python.*real_server.py" 2>/dev/null || true; echo -e "${GREEN}✅ All servers stopped${NC}"; exit 0' INT TERM
...
while true; do
    ...
    if ! curl -s http://localhost:8001/health > /dev/null 2>&1; then
        echo -e "${YELLOW}⚠️  Memory Server appears down, check logs${NC}"
    fi
