...
class LazyGraphIndexer:
    ...
    async def _initialize_embedding_model(self):
        ...
        logger.info(f"Loading LOCAL embedding model (no downloads)")
        
        try:
            # Use LOCAL embeddings - no downloads!
            from .local_embeddings import FastLocalEmbeddings
            
            # Initialize fast local embeddings
            self.embedding_model = FastLocalEmbeddings(dimension=self.dimension)
