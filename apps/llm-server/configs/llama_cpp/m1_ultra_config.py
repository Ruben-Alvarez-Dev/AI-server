...
class M1UltraConfig:
    ...
    PROFILE_MODELS: Dict[str, str] = None
    ...
    def __post_init__(self):
        ...
        if self.PROFILE_MODELS is None:
            self.PROFILE_MODELS = {
                "DEV": "Qwen2.5-Coder-7B-Instruct-Q6_K.gguf",
                "GENERAL": "Qwen2.5-7B-Instruct-Q6_K.gguf",
                "CODE": "Qwen2.5-Coder-7B-Instruct-Q6_K.gguf",
                "CHAT": "Qwen2.5-7B-Instruct-Q6_K.gguf",
                "DEFAULT": "Qwen2.5-7B-Instruct-Q6_K.gguf"
            }
    ...
    def get_model_by_profile(self, profile: str) -> str:
        """Get model filename by profile"""
        return self.PROFILE_MODELS.get(profile.upper(), self.PROFILE_MODELS["DEFAULT"])
    
    def detect_profile_from_context(self, query: str = "", context: str = "") -> str:
        """Auto-detect profile from query/context"""
        text = f"{query} {context}".lower()
        
        # Development/coding indicators
        code_keywords = ['function', 'class', 'import', 'def ', 'var ', 'const ', 'let ', 
                        'if (', 'for (', 'while (', 'try {', 'catch', 'return', 'console.log',
                        'print(', 'debug', 'error', 'exception', 'api', 'endpoint', 'http',
                        'database', 'sql', 'query', 'model', 'controller', 'service']
